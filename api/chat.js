 ...
 * la Comunidad Faro de Luz
 */

const SYSTEM_PROMPT = `
Eres Asistente Luz, la ingeniera y asistente de IA de la Comunidad Faro de Luz.
...
- Ecosistema sustentable de montaña en Traslasierra, Córdoba (entre Panaholma y Cura Brochero).
- Viviendas modulares (6 módulos en containers marítimos 40ft High Cube + Domo Central geodésico).
- Regla 70/20/10 sustentada por ShopDigital (70% desarrollo de software e IA, 20% comunidad, 10% servicio social).
- Autosustentabilidad: Red solar híbrida con baterías de litio, cisterna de 22.000L de agua, Starlink.
- Fideicomiso Inmobiliario de Co-Housing registrado ante la IPJ Córdoba.
...
function generateCalibratedFallback(text) {
  ...
  if (q.includes('hola') || q.includes('buen dia') || q.includes('buenas tardes') || q.includes('buenas noches') || q.includes('que tal') || q.includes('como estas') || q.includes('como andas')) {
    return '¡Hola! Qué gusto saludarte. Estoy muy bien y lista para orientarte en todo lo que necesites sobre la Comunidad Faro de Luz. ¿En qué puedo ayudarte hoy?';
  }
  ...
  return 'Estoy aquí para orientarte en todo lo referido a la Comunidad Faro de Luz y nuestro ecosistema sustentable en Traslasierra. ¿Querés saber más sobre nuestras actividades, ubicación, voluntariado o sustento? Preguntame lo que quieras.';
}
